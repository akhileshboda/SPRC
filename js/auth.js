...
const Auth = (() => {
  ...
  async function getGuardianParticipant() {
    const result = await api('/api/guardian/participant');
    return result.success ? result.data.participant : null;
  }

  async function updateGuardianParticipant(data) {
    const result = await api('/api/guardian/participant', {
      method: 'PUT',
